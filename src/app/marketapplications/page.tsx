...
const ImageModal: React.FC<ImageModalProps> = ({
  ...
  const downloadImage = async () => {
    try {
      ...
      link.href = url;
      link.download = `${marketName}_${currentImage.type}_${
        currentIndex + 1
      }.jpg`;
      ...
      <div className="bg-white border border-gray-200 rounded-xl max-w-md w-full shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-4">
          <p className="text-sm text-gray-600 mb-3">
            <strong className="text-gray-900">{marketName}</strong> market
...
export default function MarketApplicationsPage() {
  ...
  const [rejectionModal, setRejectionModal] = useState({
    isOpen: false,
    application: null as MarketApplication | null,
  });
  ...
  const openImageModal = (
    ...
    if (images.length > 0) {
      setImageModal({
        ...
        images,
        currentIndex: 0,
        marketName: application.name,
      });
    }
  };

  const approveApplication = async (application: MarketApplication) => {
    setProcessing(true);
    try {
      ...
      await updateDoc(doc(db, "marketApplications", application.id), {
        status: "approved",
      });

      await updateDoc(doc(db, "users", application.ownerId), {
        [`memberOfMarkets.${marketRef.id}`]: "owner",
        verified: true,
      });
      ...
      await addDoc(
        collection(db, "users", application.ownerId, "notifications"),
        {
          ...
          isRead: false,
          message: "Tap to visit your market.",
          message_en: "Tap to visit your market.",
          message_tr: "Marketinizi ziyaret etmek için dokunun.",
          message_ru: "Нажмите, чтобы посетить свой маркет.",
        }
      );
      ...
      // Log admin activity
      logAdminActivity("Market başvurusu onaylandı", {
        marketName: application.name,
      });
  ...
  const handleReject = (application: MarketApplication) => {
    setRejectionModal({
      isOpen: true,
  ...
  const handleRejectConfirm = async (rejectionReason: string) => {
    ...
    try {
      ...
      await addDoc(
        collection(db, "users", application.ownerId, "notifications"),
        {
          type: "market_disapproved",
          timestamp: serverTimestamp(),
          isRead: false,
          message: "Your market application has been rejected.",
          message_en: "Your market application has been rejected.",
          message_tr: "Market başvurunuz reddedildi.",
          message_ru: "Ваша заявка на маркет была отклонена.",
          rejectionReason: rejectionReason,
        }
      );

      // Log admin activity
      logAdminActivity("Market başvurusu reddedildi", {
        marketName: application.name,
      });
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-gray-50">
          ...
          {!loading && applications.length === 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-500">
                Henüz onay bekleyen market başvurusu bulunmamaktadır.
              </p>
            </div>
          )}
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              ...
              <div className="bg-gray-50 border-b border-gray-200 px-6 py-3">
                <div className="grid grid-cols-12 gap-4 text-xs font-medium text-gray-700 uppercase tracking-wide">
                  ...
                  <div className="col-span-2">Market Bilgileri</div>
        ...
        <ImageModal
          ...
          marketName={imageModal.marketName}
        />
        ...
        <RejectionModal
          ...
          marketName={rejectionModal.application?.name || ""}
